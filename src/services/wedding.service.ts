...
export const weddingService = {
  ...
  publishWedding: async (id: string): Promise<any> => {
    const response = await apiService.post(API_ENDPOINTS.WEDDING.PUBLISH, { id });
    return response.data;
  },
  getWeddingById: async (id: string): Promise<any> => {
    const response = await apiService.post(API_ENDPOINTS.WEDDING.FIND_BY_ID, { id });
    return response.data;
  },
  getWeddingBySlug: async (slug: string): Promise<any> => {
    const response = await apiService.get(`${API_ENDPOINTS.WEDDING.FIND_BY_SLUG}/${slug}`);
    return response.data;
  },
  resolveMapUrl: async (url: string): Promise<any> => {
    const response = await apiService.post(API_ENDPOINTS.MAP.RESOLVE_URL, { url });
    return response.data;
  },
};
