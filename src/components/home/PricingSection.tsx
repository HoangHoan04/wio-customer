  ...
  return (
    <section className="py-28 px-6 relative overflow-hidden border-b border-[#d4af37]/10">
      ...
      <div className="max-w-300 mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch max-w-275 mx-auto">
          {PLANS.map((plan, i) => (
            <ScrollReveal key={i}>
              <div
                className={`group relative h-full flex flex-col p-8 rounded-2xl border transition-all duration-500 hover:-translate-y-1 overflow-hidden ${
                  plan.isPopular
                    ? "bg-[#221019]/60 border-[#d4af37] shadow-[0_15px_45px_rgba(34,16,25,0.6)]"
                    : "bg-transparent border-[#d4af37]/15 hover:border-[#d4af37]/30"
                }`}
              >
                {plan.isPopular && (
                ...
                <div className="mb-6 relative z-10 border-b border-[#d4af37]/10 pb-6 text-left">
                  <span className="text-[10px] tracking-[3px] text-[#c9a98a]/50 font-bold uppercase block mb-1">
                    {plan.subtitle}
                  </span>
                  <h3
                  ...
                  <div className="flex items-baseline gap-1.5 text-white">
                    <span
                      className="text-4xl font-light tracking-tight text-[#f5c842]"
                      style={{ fontFamily: "'Cormorant Garamond', serif" }}
                    >
                      {plan.price}
                    </span>
                    <span className="text-xs text-[#c9a98a]/60 font-light">
                      / {plan.period}
                    </span>
                  </div>
                  <p className="text-[12px] text-[#c9a98a]/70 leading-relaxed font-light mt-4">
                    {plan.desc}
                  </p>
                </div>

                <ul className="flex-1 space-y-3.5 mb-8 relative z-10 text-left">
                  {plan.features.map((feature, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-2.5 text-[12.5px] text-[#c9a98a]/80 font-light"
                    >
                      <Check
                        size={14}
                ...
                <div className="mt-auto relative z-10">
                  <Link
                    href={plan.ctaLink}
                    className={`w-full py-3.5 rounded-lg text-xs font-bold tracking-[0.2em] uppercase flex items-center justify-center transition-all duration-300 ${
                      plan.isPopular
