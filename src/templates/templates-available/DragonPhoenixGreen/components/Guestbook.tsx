...
import dragon from "@/assets/decorations/dragon_phoenix_green/dragon.webp";
import phoenix from "@/assets/decorations/dragon_phoenix_green/phoenix.webp";
...
export const Guestbook = ({
  ...
  useEffect(() => {
    if (!data?.guestbookFloating || messages.length === 0) {
      setActiveFloaters([]);
      return;
    }
  ...
  return (
    ...
    >
      ...
      {data?.guestbookFloating && messages.length > 0 && (
        <style>{`
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full justify-center items-start mt-4">
          <div className="w-full lg:col-span-1 flex flex-col gap-4 bg-white/5 p-5 md:p-6 rounded-2xl border border-white/10 backdrop-blur-sm shadow-xl relative z-20">
            ...
            <button
              ...
              className="w-full py-3 rounded-lg font-bold tracking-widest uppercase shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
              style={{
