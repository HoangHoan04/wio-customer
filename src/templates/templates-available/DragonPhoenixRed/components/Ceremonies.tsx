import { getDayStr, getLunarDateStr, getMonthStr, getWeekday, getYearStr } from "@/common/helpers";
import type { ThemeTemplateConfig } from "@/dto/theme.dto";

export const Ceremonies = ({ data, config }: { data?: any; config: ThemeTemplateConfig }) => {
      ...
      <div className="flex flex-col items-center w-full mb-2 relative z-10">
        <div className="flex flex-col gap-12 w-full px-4">
          {data.events.map((event: any, index: number) => {
            const eventLunarDateStr = event.lunarDate || getLunarDateStr(event.date);
              ...
              <div key={index} className="flex flex-col items-center w-full">
                ...
                <div className="flex flex-col items-center mb-8">
                  ...
                  >
                    ...
                    <span className="h-6 w-px opacity-30" style={{ backgroundColor: config.colors.textPrimary }}></span>
                    <span
                    ...
                    <span className="h-6 w-px opacity-30" style={{ backgroundColor: config.colors.textPrimary }}></span>
                    <span className="font-bold">THÁNG {getMonthStr(event.date)}</span>
                  </div>
