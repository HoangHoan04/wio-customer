...
export const Countdown = ({
  ...
  const TimeBox = ({ value, label }: { value: number; label: string }) => (
    <div
      className="flex flex-col items-center justify-center bg-white/5 border rounded-xl w-16 h-20 md:w-20 md:h-24 shadow-sm"
    ...
    >
      ...
      <span
        className="text-[10px] md:text-xs uppercase tracking-widest opacity-80"
        style={{
  ...
  return (
    <div className="relative flex flex-col items-center justify-center text-center w-full overflow-hidden">
      <img
        ...
        alt=""
        aria-hidden="true"
        className="absolute left-0 top-1/2 -translate-y-1/2 w-20 md:w-36 opacity-10 pointer-events-none -translate-x-6 select-none z-0"
      />
      <h3
