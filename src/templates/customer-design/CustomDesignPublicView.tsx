...
interface CustomDesignPublicViewProps {
  customDesign: unknown;
}

export default function CustomDesignPublicView({
  customDesign,
}: CustomDesignPublicViewProps) {
  ...
  useEffect(() => {
    ...
    if (customDesign) {
      try {
        const parsed =
          typeof customDesign === "string" ? JSON.parse(customDesign) : customDesign;
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (mobile) {
        ...
        setScale(Math.min(1, availableWidth / 440));
      }
    };
    handleResize();
